    ...
    try {
        ...
        tasks = await Promise.all(
            tasks.map(async (task) => {
                const completedTasks = task.todoChecklist.filter(
                    (item) => item.completed
                ).length;
...
const updateTaskChecklist = async (req, res) => {
    try {
        ...
        //replace the existing checklist with the new one
        const completeCount = task.todoChecklist.filter((item) => item.completed).length;
        const totalItem = task.todoChecklist.length
        task.progress = totalItem > 0 ? Math.round((completeCount / totalItem) * 100) : 0
